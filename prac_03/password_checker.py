...
def is_valid_password(password):
    ...
    if not MIN_LENGTH <= len(password) and (len(password) <= MAX_LENGTH):
        print(f"It must be between{MIN_LENGTH} and {MAX_LENGTH}")
    ...
    if number_of_lower_case <= 0 or number_of_upper_case <= 0 or number_of_digit <= 0:
        return False
